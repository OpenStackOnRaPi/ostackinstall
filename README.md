...
In this repo, we describe how to install OpenStack on Raspberry Pi cluster using Kolla-Ansible. The main field of application of such clusters is instructional tools for teaching. While there are other guides on this topic available in the Internet, they mostly use an old version of OpenStack and fail to document a number of important details that would make your OpenStack Rapberry Pi cluster run reliably.

A number of OpenStack cluster configurations have been tested thoroughly. Each configuration was determined by Raspberry Pi board type (4B or 5), Kolla-Ansible OpenStack release (2023.1 or 2025.1), virtualization type (Qemu emulation or KVM). In all configurations, the RPis run under Raspberry Pi OS (Debian 12 Bookworm derivative). We confirm that KVM virtualization works excellent on RPi 4 and RPi 5 for both tested releases of Kolla-Ansible/OpenStack (2023.1 and 2025.1). Qemu only works well on RPi 4 , and the VMs are more than twice as slow compared to when host-passthrough KVM is used (Qemu needs extra settings to be added to globals.yml and nova-compute.conf Kolla-Ansible configuration files). Qemu doesn't work on RPi 5 because there is no support for the Cortex-A76 processor model in the libvirt package installed by Kolla-Ansible (we base this conclusion on our analysis of libvirtd logs). Considering the above, KVM is recommended for OpenStack deployments on the Raspberry Pi platform. Moreover, in all cases tested we had to increase the swap memory size on the OpenStack control node from 512 MB (being the default value in the Raspberry Pi OS) to a much safer capacity of 4 GB.
...
1. Raspberry Pi 4 and Raspberry 5
   ...
   * An illustrative diagram of the cluster with Linksys as the router is shown below:
...
3. Network:
   ...
   * **reserve a pool of IP addresses for the use by OpenStack** on your local router; 20 addresses will be sufficient for our purposes. They **must not** be managed by the DHCP server. Four of them (two in case of two-board cluster)) will be assigned by you to the RPis using netplan (see [Configuring the network (flat)](#configuring-the-network-flat) in subsection 3.ii), and one will be allocated as the so-called ```kolla_internal_vip_address``` (see [section 5.iii](#5iii-configure-kolla-ansible-files-for-specific-openstack-depolyment)). Remaining addresses will serve as ```floating IP addresses``` for accessing created instances from the outside of your cloud.
4. Virtualization
