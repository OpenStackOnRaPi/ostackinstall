...
As some people may be interested only in basic networking, while the others in more advanced topics, we split the description into two tracks. One track relates to pure flat provider network and the other covers VLAN (tagged) provider networks. If you want to use flat provider networks only, follow the remainder of this subsection 3.iii and then continue with sections 3.iv, [4. Management host preparation](#4-management-host-preparation), [5. Kolla-ansible and OpenStack installation](#5-kolla-ansible-and-openstack-installation) and [6. Managing your cluster](#managing-your-cluster).
