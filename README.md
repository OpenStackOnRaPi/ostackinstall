...
3. Physical network:
   ...
   * **reserve a pool of IP addresses for the use by OpenStack** on your local router (Linksys in the diagram shown above); 15 addresses will be sufficient for our purposes. They **must not** be managed by the DHCP server on your local router. Some of them will be assigned by you to the RPis using netplan (see [Configuring the network (flat)](#configuring-the-network-flat) in subsection 3.ii), and one will be allocated as the so-called ```kolla_internal_vip_address``` (see [section 5.iii](#5iii-configure-kolla-ansible-files-for-specific-openstack-depolyment)). The remaining addresses will serve as OpenStack `floating IP addresses` allowing access to created virtual machines from outside the cloud. (We assume you will not create more that 10 virtual machines, so a pool of 15 reserved addresses should be enough.)
4. Virtualization
