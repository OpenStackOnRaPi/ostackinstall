  ...
  * run init-runonce script that creates external network, subnetwork, router, downloads img image of cirros VM, and creates a couple of flavors in the admin project
    - init-runonce script files for various kolla-ansible/OpenStack releases are available under following links. Select appropriate os-name, possibly editing it to change some settings according to your environment (e.g., external network address range):
      ...
      - [init-runonce.2024.1](/init-runonce.2024.1)
        Note: currently tested and recommended settings are `<kolla-ansible-release>=2023.1` and `<os-name>=cirros`
     
      (add link to cirros script)
      ```bash
      ./init-runonce.2023.1.cirros
      ```
      Note: the original version of init-runonce script is generated by the command `kolla-ansible post-deploy`. In our lab, we use slightly modified version of it to match our Raspberry Pi environmanet requirements (e.g., we need aarch64 image, we can afford rather small VM flavors, etc.).
