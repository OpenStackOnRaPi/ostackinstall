...
As of mid July 2025, cluster configurations have been tested thoroughly for Kolla-Ansible OpenStack release `2023.1` and `2025.1`, assuming Raspberry Pi 4B and Raspberry Pi 5 as the hardware platform, and Raspberry Pi OS (Debian 12). For each such configuration mentioned, OpenStack has been tested in two virtualization variants: with QEMU emulation and with KVM enabled. KVM virtualization works excellent on RPi 4 and RPi 5, and for both tested releases of Kolla-Ansible/OpenStack (2023.1, 2025.1). Qemu works well only on RPi 4, althoug the VMs are over than two times slower compared to when KVM host-passthrough is used. Qemu doesn't work on RPi 5 because there is no support for the Cortex-A76 processor model in the libvirt package installed by Kolla-Ansible (this opinion is based on our analysis of libvirtd logs). In all options we had to increase the swap memory size on the OpenStack control node from 512 MB being the default value in the Raspberry Pi OS to a much safer capacity of 4 GB.

In summary, both the Raspberry Pi 4 and 5 are great platforms for setting up small and cheap, bare metal OpenStack clusters for teaching purposes. To achieve greater platform uniformity, we will ultimately strive to use only one version – Kolla-Ansible 2025.1. Nevertheless, the current (July 20, 2025) version of this manual still explicitly refers to version 2023.1, which should be taken into account when one installs version 2025.1. It will be be updated as new results or recommendations emerge.
