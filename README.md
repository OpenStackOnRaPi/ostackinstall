...
In summary, both the Raspberry Pi 4 and 5 are suitable platforms for setting up small OpenStack clusters for teaching purposes, with the Raspberry Pi 5 requiring a little bit more testing. To achieve greater platform homogenity, we will target at recomminding for use only release 2025.1. Nevertheless, current (July 20th 2025) version of this guide continues to address Kolla-Ansible release 2023.1 which has to be accounted for if one wants to use release 2025.1. The latter is going to be updated soon. We'll also update this guide as new results or recommendations emerge.
