...
There are two main options for installing ZroTier in your case.

If you have a spare (dedicated) computer to host the VPN connection to the cluster you cas use it as the access node to your cluster (jump host). If your local router has been configured, and your Raspberry Pis are up and running with the newly installed SD cards, everyone in your team will be able to complete all the remaining steps in this guide. Note that such a jump host has to be connected to the L2 network segment of your OpenStack.
