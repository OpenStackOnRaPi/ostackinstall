...
**NOTE: this setup is prepared for a flat provider network only in the OpenStack DC. To allow for VLAN provider networks, additional configurations are needed for ```eth0```, ```brmux``` and ```veth1br``` to declare the allowed ranges of VLAN ids that can be used. If you are interested in VLAN provider networks, contact the instructor for more info.**
