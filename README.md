...
2. First checks - create first instance
  ...
  * run init-runonce script that creates external network, subnetwork, router, downloads img image of cirros VM, and creates a couple of flavors in the admin project
    - init-runonce script files for various kolla-ansible/OpenStack releases are available under following links. Select appropriate os-name, possibly editing it to change some settings according to your environment (e.g., external network address range):
      ...
      Note 2: the original version of init-runonce script is generated by the command `kolla-ansible post-deploy`. In our lab, we use slightly modified version of it to match our Raspberry Pi environmanet requirements (e.g., we need aarch64 image, we can afford rather small VM flavors, etc.).
      
      Note 1: currently tested and recommended settings are `<kolla-ansible-release>=2023.1` and `<os-name>=cirros`, so the recommended script is available here:
      add the link ?????????
    ...
    After running `./init-runonce.20xy.z` the external and tenant networks, VM image, VM flavors and the default security group settings are all ready. The first instance can be created in the following two steps:
