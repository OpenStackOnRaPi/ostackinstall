...
A number of OpenStack cluster configurations have been tested thoroughly. Each configuration was determined by Raspberry Pi board type (4B or 5), Kolla-Ansible OpenStack release (2023.1 or 2025.1), virtualization type (Qemu emulation or KVM). In all configurations, the RPis run under Raspberry Pi OS (Debian 12 Bookworm derivative). We confirm that KVM virtualization works excellent on RPi 4 and RPi 5 for both tested releases of Kolla-Ansible/OpenStack (2023.1 and 2025.1). Qemu only works well on RPi 4 , and the VMs are more than twice as slow compared to when host-passthrough KVM is used (Qemu needs adding extra settings in globals.yml and nova-compute.cong Kolla-Ansible configuration files). Qemu doesn't work on RPi 5 because there is no support for the Cortex-A76 processor model in the libvirt package installed by Kolla-Ansible (we base this conclusion on our analysis of libvirtd logs). Considering the above, KVM is recommended for OpenStack deployments on the Raspberry Pi platform. Moreover, in all cases tested we had to increase the swap memory size on the OpenStack control node from 512 MB (being the default value in the Raspberry Pi OS) to a much safer capacity of 4 GB.
