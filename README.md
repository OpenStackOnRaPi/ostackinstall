...
As of mid July 2025, cluster configurations have been tested quite thoroughly for Kolla-Ansible OpenStack release `2023.1` and for Raspberry Pi 4B and Raspberry Pi 5 as the hardware platform. For each SW/HW configuration mentioned, OpenStack was tested in two variants: with QEMU emulation and with KVM enabled. Both variants work well on RPi4 and RPi5 and release 2023.1 of Kolla-Ansible/OpenStack, with KVM being the prefered choice (QEMU works, too). We have only increased the swap size on the control node of OpenStack from the default value 512MB (in Raspberry Pi OS) to much safer capacity 4GB. We are currently testing release 2025.1 in a cluster containing three RPi 5 hosts and KVM virtualization, and it works stably, too. Thus, also this combination seems to be a good choice (although QEMU does not work in this case because of lack of support for the cortex-A76 CPU model in the libvirt package installed by Kolla-Ansible).

In summary, both the Raspberry Pi 4 and 5 are suitable platforms for setting up small OpenStack clusters for teaching purposes, with the Raspberry Pi 5 requiring a little bit more testing. To achieve greater platform uniformity, we will ultimately strive to use only one version – Kolla-Ansible 2025.1. Nevertheless, current (July 20th 2025) version of this guide continues to explicitly address release 2023.1 which has to be accounted for when using release 2025.1. The latter is going to be updated soon, though. We'll also update this guide as new results or recommendations emerge.
