...
In this repo, we describe how to install OpenStack on Raspberry Pi cluster using Kolla-Ansible. Although there are other guides out there on this topic, they either use old version of OpenStack or do not document several nitty-gritty details needed for OpenStack cluster to work sufficiently reliably on the Rapberry Pi platform. The main application of such clusters is instructional tools for teaching.

As of mid July 2025, cluster configurations have been tested quite thoroughly for Kolla-Ansible OpenStack release `2023.1` and for Raspberry Pi 4B and Raspberry Pi 5 as the hardware platform. For each SW/HW configuration mentioned, OpenStack was tested in two variants: with QEMU emulation and with KVM enabled. Both variants work well on RPi4 and RPi5 and release 2023.1 of Kolla-Ansible/OpenStack, with KVM being the prefered choice (QEMU works, too). We only increased the swap size on the OpenStack control node from the default value of 512 MB (in the Raspberry Pi OS) to a much safer capacity of 4 GB. We're currently testing version 2025.1 in a cluster with three RPi 5 hosts and KVM virtualization. It works reliably, so this option also seems worth recommending (QEMU doesn't work in this case because there is no support for the Cortex-A76 processor model in the libvirt package installed by Kolla-Ansible).

In summary, both the Raspberry Pi 4 and 5 are suitable platforms for setting up small OpenStack clusters for teaching purposes, with the Raspberry Pi 5 requiring a little bit more testing. To achieve greater platform uniformity, we will ultimately strive to use only one version – Kolla-Ansible 2025.1. Nevertheless, the current (July 20, 2025) version of this manual still explicitly refers to version 2023.1, which should be taken into account when using version 2025.1. However, this document will be updated soon. We'll also update it as new results or recommendations emerge.
...
1. Raspberry Pi 4 and Raspberry 5
   * tested options: [2x4GB RAM + 2x8GB RAM] or [3/4x8GB RAM] per cluster (uniform 8GB RAM clusters are better)
     ...
     * control/netowerk node should run on 8GB RPi host; network node must run on another RPi host (control / network nodes can be split between different hosts, nevertheless this results in a very limited decrease of host peak RAM utilization in the cluster)
   * all Pi are equipped with 32GB SD disk
   * Note: a single 8GB RAM RPi host in all-in-one setup of Kolla-Ansible OpenStack is able to host OpenStack in absolutely minimal configuration. In such a cluster, one can create a single CirrOS instance with 512MB memory, but the cloud becomes unstable. We have seen many times that even such a simple configuration ends in failure. And all-in-one OpenStack immediately crushes due to out of memory when a second similar CirrOS instance is created unless you increase the swap at the cost of things becoming very slow. That is the reason why we consider the dual-host configuration to be the minimum possible.
2. SW
